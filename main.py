...
option = {
    ...
    "Linux": {
        "Apt": {
            ...
            "Creative": [["LAME for Audacity", "lame"], ],
            "Dev Langs & Runtimes": [["Python 3", "python3"],
                                     ["Python2", "python2"],
            ...
            "Communication": [["Thunderbird", "thunderbird"], ],
            "Security": [["KeePassXC", "keepassxc"],
                         ["QtPass", "qtpass"],
                         ["OpenVPN", "openvpn"],
                         ["WireGuard", "wireguard"], ],
            "Utility": [["Flameshot", "flameshot"], ],
            "Writing": [["LibreOffice", "libreoffice"], ]
        },
        "Flatpak": {
            "Browsers": [["Mozilla Firefox", "org.mozilla.firefox"],
            ...
            "Cloud": [["Dropbox", "com.dropbox.Client"],
                      ["Nextcloud", "com.nextcloud.desktopclient.nextcloud"], ],
            "Compression": [["PeaZip", "io.github.peazip.PeaZip"], ],
            "Creative": [["OBS Studio", "com.obsproject.Studio"],
            ...
            "Dev Tools": [["Visual Studio Code", "com.visualstudio.code"],
            ...
            "File Sharing": [["qBittorrent", "org.qbittorrent.qBittorrent"],
                             ["Transmission", "com.transmissionbt.Transmission"],
                             ["Deluge", "org.deluge_torrent.deluge"], ],
            "Gaming": [["Steam", "com.valvesoftware.Steam"],
                       ["Heroic Games Launcher", "com.heroicgameslauncher.hgl"],
                       ["Lutris", "net.lutris.Lutris"], ],
            "Media": [["Spotify", "com.spotify.Client"],
                      ["VLC", "org.videolan.VLC"],
                      ["Handbrake", "fr.handbrake.ghb"], ],
            "Communication": [["Discord", "com.discordapp.Discord"],
                              ["Slack", "com.slack.Slack"],
                              ["Zoom", "us.zoom.Zoom"],
                              ["Skype", "com.skype.Client"],
                              ["Thunderbird", "org.mozilla.Thunderbird"], ],
            "Security": [["Bitwarden", "com.bitwarden.desktop"],
                         ["KeePassXC", "org.keepassxc.KeePassXC"],
                         ["Keepass", "org.keepassxc.KeePassXC"], ],
            "Utility": [["Flameshot", "org.flameshot.Flameshot"], ],
            "Writing": [["Obsidian", "md.obsidian.Obsidian"],
        ...
        "Yay": {
            ...
            "Cloud": [["Dropbox", "dropbox"],
                      ...
                      ["Nextcloud", "nextcloud"],
                      ["Syncthing (CLI)", "syncthing"], ],
            "Compression": [["7-Zip", "7-zip-full"],
                            ["PeaZip", "peazip-qt-bin"], ],
            "Creative": [["OBS Studio", "obs-studio"],
                         ["LAME for Audacity", "lame"],
                         ["Unreal Engine", "unreal-engine"],
                         ["Godot", "godot"],
                         ["Godot (with Mono/C#)", "godot-mono-bin"], ],
            "Dev Langs & Runtimes": [["Python 3", "python"],
                                     ["Python2", "python2"],
                                     ["NodeJS", "nodejs"],
                                     ...
                                     ["PHP", "php"],
                                     ["Perl", "perl"], ],
            "Dev Tools": [["PowerShell", "powershell-bin"],
            ...
            "Documents": [["Foxit PDF Reader", "foxitreader"], ],
            "Gaming": [["Steam", "steam"],
            ...
            "Utility": [["Etcher", "etcher-bin"],
                        ["Flameshot", "flameshot"], ],
            "Writing": [["Notion", "notion-app-electron"],
    ...
    if os == "windows":
        if package_manager == "choco":
            while True:
                print("\x1B[2J")
                counter = 1
                for sublist in option[os.capitalize()][package_manager.capitalize()][category_dic[category2]]:
        ...
        Press enter to return back to main menu
                ...
                if not choice.__contains__("desc:") and not choice == "":
                    return Choco_download.append(
                        option[os.capitalize()][package_manager.capitalize()][category_dic[category2]][
                            int(choice) - 1
                            ][1]
                    )
        ...
        Press enter to return back to main menu
                ...
                choice = input("Enter the corresponding number...")
                if not choice.__contains__("desc:") and not choice == "":
                    choice = option["Linux"]["Winget"][category_dic[category2]][int(choice) - 1][1]
                    return Yay_download.append(choice)
                elif choice.__contains__("desc:"):
                    print("I could not find any information about how to view details about a program in Yay")
                    print("Because of that this feature is not available for this package manager")
                    choice = input("To return back press enter...")
                elif choice == "":
                    category2 = ""
                    break
...
if plat == "Windows":
        ...
        print("Please note that if it is not first time you are logging in to Windows, Winget must be installed \n")
        ...
        if source == "1" or source.capitalize() == os_list[0]:
            ...
            while True:
                if (
                    ...
                    elif category.lower() == "download" and len(Scoop_download) > 1 and not Scoop_part_2:
                        subprocess.call(Scoop_part_1, shell=True)
                        Scoop_download = " ".join(Scoop_download)
                        print(Scoop_download, sep="\n")
                        subprocess.call(Scoop_download, shell=True)
                        exit()
                    elif category.lower() == "download" and len(Scoop_download) > 1 and Scoop_part_2:
                        Scoop_download = " ".join(Scoop_download)
                        print(Scoop_download, sep="\n")
                        subprocess.call(Scoop_download, shell=True)
                        exit()
        ...
        if source == "1" or source.capitalize() == os_list[0]:
            while True:
                ...
                elif category.lower() == "download" and len(Apt_download) > 1 and not Apt_part_2:
                    subprocess.call(Apt_part_1, shell=True)
                    Apt_download = " ".join(Apt_download)
                    print(Apt_download, sep="\n")
                    subprocess.call(Apt_download, shell=True)
                    exit()
                elif category.lower() == "download" and len(Apt_download) > 1 and Apt_part_2:
                    Apt_download = " ".join(Apt_download)
                    print(Apt_download, sep="\n")
                    subprocess.call(Apt_download, shell=True)
                    exit()
            ...
            while True:
                if (
                    ...
                    if category != "download" and int(category) in range(
                        ...
                        break
                elif isFlatpakinstalled.lower() == "n" or isFlatpakinstalled.lower() == "no":
                    print("\n Please visit the url for setup instructions (https://flatpak.org/setup/) \n")
                    isFlatpakinstalled = input("Is Flatpak installed? Y/n... ")
                    if isFlatpakinstalled.lower() == "y" or isFlatpakinstalled.lower() == "yes" or isFlatpakinstalled == "":
        ...
        while True:
            if (
                ...
                elif category.lower() == "download" and (len(selection_cask) > 1 or len(selection_formula) > 1):
